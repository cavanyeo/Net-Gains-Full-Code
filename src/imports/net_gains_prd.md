...
| Req ID | Requirement |
|---|---|
| FR-04 | The dashboard shall display a **circular progress bar** (≥ 160 px diameter) at the top/centre, filled in **Sage (#9CA288)**, showing `coins earned this week / weekly coin goal`. |
...
| FR-07 | **Node states:** ✅ Completed (filled Sage), 🔵 Current/Active (pulsing outline, Sage), ⬜ Upcoming (Pearl #CED0B7 outline), 🔒 Locked (greyed, not today). |
...
| Req ID | Requirement |
|---|---|
| FR-11 | The top of the screen shall display an **active course card** (elevated, Beige #FFFEE5 background, Dutch White #EBDCB8 border) showing: course title, week number, curriculum summary (e.g., "7 × 1-min videos · 7 quizzes · 7 challenges"), and a mini progress bar (days completed / 7). |
| FR-12 | Below the active card, the screen shall render a **vertical list of future courses**, each displayed as a card in **Pearl (#CED0B7)** with reduced opacity (0.5), a lock icon, and `pointer-events: none`. |
...
| Req ID | Requirement |
|---|---|
| FR-15 | The top of the screen shall display the user's **total coin balance** in a prominent header bar (Taupe #544739 text on Beige #FFFEE5). |
...
| FR-17 | Rewards the user cannot yet afford shall display the coin cost in **Pearl (#CED0B7)** text with a disabled state. Affordable rewards shall display the cost in **Sage (#9CA288)** with an active "Redeem" button. |
...
| FR-21 | The active tab icon shall use **Sage (#9CA288)**; inactive tabs shall use **Pearl (#CED0B7)**. |
...
| Token | Hex | Usage |
|---|---|---|
| **Rich Black** | `#0B0D0B` | Body text, headings, dark accents, primary text on light backgrounds |
| **Mountain Meadow** | `#20C580` | Primary buttons, active states, progress fills, primary actions |
| **Caribbean Green** | `#00FFA3` | Highlights, success states, accent elements, active indicators |
| **Pistachio** | `#AACBB4` | Secondary/inactive elements, subtle backgrounds, borders |
| **Anti-Flash White** | `#F1F1F6` | Page backgrounds, card surfaces, light areas |
| **Bangladesh Green** | `#007039` | Deep accents, strong emphasis, badges |
| **Mint** | `#2DCB91` | Supplementary accents, interactive elements |
| **Stone** | `#707070` | Disabled states, subtle text, secondary information |
| **Dark Green** | `#16261B` | Dark mode backgrounds, deep shadows |

**Typography:** Axiforma (Regular, Medium, Semi Bold) — body 14 px, headings 20–28 px, weights 400/500/600.  
**Corner Radius:** 16 px (cards), 12 px (buttons), 50% (avatar, progress ring).  
**Elevation:** Subtle box-shadows with a slight 3D feel (inspired by LumiHealth and Bevel).  
