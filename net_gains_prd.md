...
3. **Task Initiation** — If the task is available, today's node pulses. Tapping it opens the task modal showing the video, quiz, and practical challenge.
4. **Video Consumption** — The user must watch ≥ 90% of the video's duration. A progress bar on the video player tracks this. The quiz and challenge buttons remain disabled until the threshold is met.
5. **Quiz + Challenge Attempt** — The quiz consists of 5 MCQ questions; the challenge is a practical task (e.g., save $5, draft a budget). The combined score must be ≥ 80% to pass.
...
7. **Fail → Immediate Retry** — If the user fails, the app displays an **encouraging message** (e.g., "Almost there! Let's review the video and try again") and **reroutes them to rewatch the video**, after which the quiz and challenge unlock again. There is **no lockout** — the user may retry as many times as needed within the day, with diminishing coin rewards per retry.
